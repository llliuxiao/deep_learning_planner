...
sys.path.append(f"/home/{os.getlogin()}/barn_ws/src/deep_learning_planner/scripts")
...
class TransformerPlanner:
    ...
    def make_tensor(self):
        ...
        goal = (self.global_plan.poses[-1].pose.position.x,
                self.global_plan.poses[-1].pose.position.y,
                self._get_yaw(self.global_plan.poses[-1].pose.orientation))
        ...
        if self.flag == "reinforcement":
            ...
            laser_mask = torch.unsqueeze(laser_mask, 0).to(self.device)
            return laser_tensor, global_plan_tensor, goal_tensor, laser_mask
...
if __name__ == "__main__":
    ...
    planner = TransformerPlanner(
        flag="imitation",
        model_file=imitation_model_file,
        velocity_factor=1.5,
        world_frame="odom",
        robot_frame="base_link",
        scan_topic_name="/front/scan",
